...
export function AddChannelModal({ allGroups, onClose, onAdd }: Props) {
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-visible border border-slate-100 animate-in zoom-in-95 duration-200 text-slate-900 flex flex-col max-h-[90vh]">
        <div className="px-8 py-6 border-b flex justify-between items-center bg-white shrink-0 rounded-t-[2rem]">
          <h3 className="text-xl font-extrabold text-slate-800">添加新频道</h3>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input name="tvgId" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl outline-none text-xs text-slate-600 focus:ring-2 focus:ring-blue-500 transition-all shadow-sm focus:bg-white" placeholder="例如: CCTV1" />
            </div>
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-400 uppercase ml-1">TVG-Name (内部名称)</label>
              <input name="tvgName" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl outline-none text-xs text-slate-600 focus:ring-2 focus:ring-blue-500 transition-all shadow-sm focus:bg-white" placeholder="EPG 匹配名称 (选填)" />
            </div>
          </div>
          
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-400 uppercase ml-1">Logo 地址</label>
            <input name="tvgLogo" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl outline-none text-xs text-slate-600 focus:ring-2 focus:ring-blue-500 transition-all shadow-sm focus:bg-white" placeholder="http://example.com/logo.png" />
          </div>
          
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-400 uppercase ml-1">直播源 (URL)</label>
            <textarea name="url" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-mono text-xs h-28 resize-none text-slate-600 focus:ring-2 focus:ring-blue-500 transition-all shadow-sm focus:bg-white" required placeholder="http://example.com/stream.m3u8" />
          </div>
          ...
          <div className="pt-4 flex justify-end gap-3">
            ...
            <button type="submit" className="px-10 py-3 bg-slate-900 text-white rounded-2xl font-bold shadow-xl shadow-slate-200 hover:bg-slate-800 transition-all active:scale-95 text-sm">确认添加</button>
          </div>
        </form>
      </div>
    </div>
