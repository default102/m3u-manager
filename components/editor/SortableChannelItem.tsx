  ...
  return (
    <div ref={setNodeRef} style={style} className={`bg-white dark:bg-slate-900 border border-transparent dark:border-slate-800/30 ${selected ? 'ring-2 ring-blue-500 bg-blue-50/10' : 'hover:bg-slate-50 dark:hover:bg-slate-850'} p-3 md:p-4 rounded-xl flex items-center gap-3 md:gap-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 group relative text-slate-900 dark:text-slate-100`}>
      <div 
        {...attributes} 
        {...listeners} 
        className={`p-1 text-slate-300 dark:text-slate-700 transition-colors z-10 ${isAllView ? 'cursor-not-allowed opacity-10' : 'cursor-grab hover:text-slate-500 dark:hover:text-slate-400'}`} 
        style={{ touchAction: isAllView ? 'auto' : 'none' }}
      >
        <GripVertical size={16} />
      </div>
      
      <button onClick={() => onToggleSelect(channel.id)} className={`p-1 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors ${selected ? 'text-blue-600 dark:text-blue-400' : 'text-slate-300 dark:text-slate-700'}`}>
         {selected ? <CheckSquare size={20} /> : <Square size={20} />}
      </button>

      <div className="w-10 h-10 bg-slate-50 dark:bg-slate-950 rounded-lg flex items-center justify-center overflow-hidden shrink-0 shadow-inner p-1">
         {channel.tvgLogo ? (
            <img src={channel.tvgLogo} alt="" className="w-full h-full object-contain" onError={(e) => (e.currentTarget.style.display = 'none')} />
         ) : (
            <span className="text-[10px] font-bold text-slate-300 dark:text-slate-700">TV</span>
         )}
      </div>
      ...
      <div className="flex-1 min-w-0 flex items-center gap-4">
        <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
                <div className={`font-bold text-sm md:text-base truncate ${isHidden ? 'text-slate-400 dark:text-slate-650 line-through' : 'text-slate-700 dark:text-slate-200'}`}>{channel.name}</div>
                {isHidden && <span className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">隐藏</span>}
            </div>
            <div className="text-xs text-slate-400 dark:text-slate-500 truncate font-mono mt-0.5 opacity-60 group-hover:opacity-100 transition-opacity">{channel.url}</div>
        </div>
      </div>
