...
export default function Home() {
  ...
  const handleCopy = (id: number) => {
    const fullUrl = `${baseUrl}/api/export/${id}`;
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(fullUrl).then(() => {
        setCopiedId(id);
        setTimeout(() => setCopiedId(null), 2000);
      }).catch(() => fallbackCopyTextToClipboard(fullUrl, id));
    } else {
      fallbackCopyTextToClipboard(fullUrl, id);
    }
  };
  ...
  const executeImportOrUpdate = async () => {
    ...
    try {
      if (updatingId) {
          const res = await fetch(`/api/playlist/${updatingId}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ url, content: fileContent })
          });
          ...
          const res = await fetch('/api/playlist', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ name, url, content: fileContent })
          });
  ...
  const handleRename = async () => {
    ...
    try {
        const res = await fetch(`/api/playlist/${showRename.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: showRename.name })
        });
        if (res.ok) {
                  ...
                  <div className="space-y-4">
                     ...
                     <div className="space-y-2">
                         ...
                         <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-slate-200 border-dashed rounded-2xl cursor-pointer bg-slate-50 hover:bg-slate-100 hover:border-blue-300 transition-all group">
                             ...
                             <input type="file" className="hidden" onChange={handleFileChange} accept=".m3u,.m3u8,.txt" />
                         </label>
                     </div>
                  </div>
